...
// Address for memory scrub related testing
#define SGL_START_ADDRESS  (volatile datum *) 	0x1a10b030
#define DBL_START_ADDRESS  (volatile datum *) 	0x1a10b034

#define SGL_VAL_ADDRESS  (volatile datum *) 	0x1a10b028
#define DBL_VAL_ADDRESS  (volatile datum *) 	0x1a10b02c

#define SGL_RESET_ADDRESS  (volatile datum *) 	0x1a10b038
#define DBL_RESET_ADDRESS  (volatile datum *) 	0x1a10b03c

#define PERIPHERAL_START_ADDRESS (volatile datum *) 0x1a100000
#define MEMORY_START_ADDRESS (volatile datum *) 	0x1c000000
    ...
    for(;;){
    	...
    	else
    	{
    		if( gSimulatorCmdTable[gSimulatorCmdTableIndex] != NULL )
    		{
    			...
    			if (gSimulatorCmdTableIndex == 18) {
    				gSimulatorCmdTableIndex = 0;

    				// changes
    				mem_scrubber();

    				CLI_printf("SEU counter: %i\n", seu_count);
					CLI_printf("DEU counter: %i\n", deu_count);
...
void mem_scrubber(void) {

	unsigned long offset;
	unsigned long nBytes = 2097151; //memory size peripheral 1F_FFFF
	unsigned long nWords =  nBytes / sizeof(datum);
	datum x;


	for (offset = 0; offset < nWords; offset++) {
		x = *(PERIPHERAL_START_ADDRESS + offset);
	}

	nBytes = 524287; // memory size of memory bank
	nWords =  nBytes / sizeof(datum);

	for (offset = 0; offset < nWords; offset++) {
		x = *(MEMORY_START_ADDRESS + offset);
	}

	seu_count = *SGL_VAL_ADDRESS;
	deu_count = *DBL_VAL_ADDRESS;
